...
public class FanaticsProcessReleaseMsgService implements YIFCustomApi {
	...
	public void processRelease(YFSEnvironment env, Document inDoc){
		...
		try{
			...
			Element eleReleaseControlNbr = SCXmlUtil.getXpathElement(inDocCloned.getDocumentElement(),
					OrderReleaseConstants.XPATH_ORDER_CUSTOM_ATTR);
			
			Element eleReleaseControlNbrClone = (Element) eleReleaseControlNbr.cloneNode(false);
			...
			if (shipTogetherList != null && shipTogetherList.getLength() > 0) {
				...
				Element OrderReleaseLevelControlNbr  = SCXmlUtil.getXpathElement(inDocCloned.getDocumentElement(), 
						OrderReleaseConstants.XPATH_CUSTOM_ATTR);
	...
	public void setReleaseControlNumber(Element orderLevelNum, Element OrderReleaseLevelNum, String releaseControlNbr ){
		orderLevelNum.setAttribute(OrderReleaseConstants.ATT_RELEASE_CTRL_NBR, releaseControlNbr);
		OrderReleaseLevelNum.setAttribute(OrderReleaseConstants.ATT_RELEASE_CTRL_NBR, releaseControlNbr);
	}
