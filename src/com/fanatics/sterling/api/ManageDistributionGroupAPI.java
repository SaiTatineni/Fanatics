...
public class ManageDistributionGroupAPI implements YIFCustomApi {
	...
	public Document processNodeForMonitoringDG(YFSEnvironment env, Document inXML) throws Exception{
		...
		logger.info("Inside processNodeForMonitoringDG");
		logger.info("Input xml is: "+ XMLUtil.getXMLString(inXML));
		...
		logger.info("Property value of RTAM DG Code is "+strRTAMDGCode);
		logger.info("Property value of Calling Organization Code is "+strCallingOrgCode);
		...
		logger.info("Input xml after setting the attributes is: "+ XMLUtil.getXMLString(inXML));

		// find out the required action
		String strOperation = rootInXML.getAttribute(FANConstants.ATT_fanatics_OPERATION);
		logger.info("Operation is: "+ strOperation);
		/**
		...
		logger.info("Output xml of getDistributionList: "+ XMLUtil.getXMLString(docOutXML));


		// check if the vendor is a part of DG already
		Element rootdocOPgetDistributionList = docOutXML.getDocumentElement();
		logger.info("First condition is: " + rootdocOPgetDistributionList.hasChildNodes());
		logger.info("Second condition is: "+strOperation.equalsIgnoreCase(VendorFeedConstants.CONSTANT_fanatics_DELETE));
		...
		if (rootdocOPgetDistributionList.hasChildNodes() && strOperation.equalsIgnoreCase(VendorFeedConstants.CONSTANT_fanatics_DELETE)){
			 ...
			 */
			logger.info("Invoking Delete Distribution");
