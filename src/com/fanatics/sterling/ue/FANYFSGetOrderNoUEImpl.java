...
/**
 * 
 * This class provides the Use Exit implementation for the YFSGetOrderNoUE The
 * order no is geenrated using sequences for Exchange and Return Orders Two
 * seperate sequences are used 1.SEQ_DSW_RETURN_ORDER_NO : Return order sequence
 * 2.SEQ_FAN_EXCHANGE_ORDER_NO : Excahnge order sequence
 * 
...
public class FANYFSGetOrderNoUEImpl implements YFSGetOrderNoUE {
	...
	public String getOrderNo(YFSEnvironment env, Map map) throws YFSUserExitException {
		...
		String documentType = (String) map.get(FANConstants.ATT_DocumentType);
		String seqNoStr     = FANConstants.NO_VALUE;
		
		try {
			
			YFCContext ctxt = (YFCContext) env;
			Statement stmt  = ctxt.getConnection().createStatement();
			ResultSet rs    = stmt.executeQuery(FANConstants.SEQ_EXISTS_SQL);
			...
			while (rs.next()) {
				if (rs.getInt(1) < 1) {
					...
					int createSeq = createOrderSeqStmt.executeUpdate(FANConstants.SEQ_CREATE_SQL);
				} else {
					log.verbose("DB Sequence found - extracting Sequence number ...");
				}
			}

			long seqNo = ((YCPContext) env).getNextDBSeqNo(FANConstants.SEQ_FANATICS_ORDER_NO);
